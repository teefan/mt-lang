...
def render_frame() -> void:
    ...
    var destination = c.SDL_FRect(
        x = 0.0,
        y = 0.0,
        w = float<-texture_width + (float<-texture_width * scale),
        h = float<-texture_height + (float<-texture_height * scale),
    )
    destination.x = (float<-window_width - destination.w) / 2.0
    destination.y = (float<-window_height - destination.h) / 2.0
