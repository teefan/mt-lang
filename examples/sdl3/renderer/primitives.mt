...
def render_frame() -> void:
    ...
    rect.w -= 60.0
    rect.h -= 60.0
    c.SDL_RenderRect(renderer, ptr_of(rect))

    c.SDL_SetRenderDrawColor(renderer, 255, 255, 0, c.SDL_ALPHA_OPAQUE)
    c.SDL_RenderLine(renderer, 0.0, 0.0, float<-window_width, float<-window_height)
    c.SDL_RenderLine(renderer, 0.0, float<-window_height, float<-window_width, 0.0)
