...
function add_added_message(which: uint, gamepad: ptr[c.SDL_Gamepad]?) -> void:
    var message: ptr[char]? = null
...
function add_axis_message(which: uint, axis: c.Uint8, value: c.Sint16) -> void:
    ...
    unsafe: c.SDL_asprintf(
            ...
            which,
            c.SDL_GetGamepadStringForAxis(c.SDL_GamepadAxis<-(int<-axis)),
            int<-value,
        )
...
function add_button_message(which: uint, button: c.Uint8, down: bool) -> void:
    ...
    unsafe: c.SDL_asprintf(
            ...
            which,
            c.SDL_GetGamepadStringForButton(c.SDL_GamepadButton<-(int<-button)),
            state_text,
        )
        ...
        if event.type_ == uint<-c.SDL_EventType.SDL_EVENT_QUIT:
            ...
            if event.type_ == uint<-c.SDL_EventType.SDL_EVENT_GAMEPAD_ADDED:
                ...
                if event.type_ == uint<-c.SDL_EventType.SDL_EVENT_GAMEPAD_REMOVED:
                    ...
                    if event.type_ == uint<-c.SDL_EventType.SDL_EVENT_GAMEPAD_AXIS_MOTION:
                        ...
                        if event.type_ == uint<-c.SDL_EventType.SDL_EVENT_GAMEPAD_BUTTON_UP or event.type_ == uint<-c.SDL_EventType.SDL_EVENT_GAMEPAD_BUTTON_DOWN:
