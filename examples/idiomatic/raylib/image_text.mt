...
def main() -> i32:
    ...
    rl.image_draw_text_ex(inout parrots, font, "[Parrots font drawing]", rl.Vector2(x = 20.0, y = 20.0), cast[f32](font.baseSize), 0.0, rl.RED)
    ...
    let position = rl.Vector2(
        x = cast[f32](screen_width) / 2.0 - cast[f32](texture.width) / 2.0,
        y = cast[f32](screen_height) / 2.0 - cast[f32](texture.height) / 2.0 - 20.0,
    )
    ...
    while not rl.window_should_close():
        ...
        if not show_font:
            ...
            rl.draw_text_ex(font, "[Parrots font drawing]", rl.Vector2(x = position.x + 20.0, y = position.y + 300.0), cast[f32](font.baseSize), 0.0, rl.WHITE)
        else:
