...
def main() -> i32:
    ...
    while not rl.window_should_close():
        ...
        rl.draw_text(if rl.is_window_state(fullscreen_flag) then "[F] FLAG_FULLSCREEN_MODE: on" else "[F] FLAG_FULLSCREEN_MODE: off", 10, 80, 10, if rl.is_window_state(fullscreen_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(resizable_flag) then "[R] FLAG_WINDOW_RESIZABLE: on" else "[R] FLAG_WINDOW_RESIZABLE: off", 10, 100, 10, if rl.is_window_state(resizable_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(undecorated_flag) then "[D] FLAG_WINDOW_UNDECORATED: on" else "[D] FLAG_WINDOW_UNDECORATED: off", 10, 120, 10, if rl.is_window_state(undecorated_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(hidden_flag) then "[H] FLAG_WINDOW_HIDDEN: on" else "[H] FLAG_WINDOW_HIDDEN: off (hides for 3 seconds)", 10, 140, 10, if rl.is_window_state(hidden_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(minimized_flag) then "[N] FLAG_WINDOW_MINIMIZED: on" else "[N] FLAG_WINDOW_MINIMIZED: off (restores after 3 seconds)", 10, 160, 10, if rl.is_window_state(minimized_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(maximized_flag) then "[M] FLAG_WINDOW_MAXIMIZED: on" else "[M] FLAG_WINDOW_MAXIMIZED: off", 10, 180, 10, if rl.is_window_state(maximized_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(unfocused_flag) then "[U] FLAG_WINDOW_UNFOCUSED: on" else "[U] FLAG_WINDOW_UNFOCUSED: off", 10, 200, 10, if rl.is_window_state(unfocused_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(topmost_flag) then "[T] FLAG_WINDOW_TOPMOST: on" else "[T] FLAG_WINDOW_TOPMOST: off", 10, 220, 10, if rl.is_window_state(topmost_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(always_run_flag) then "[A] FLAG_WINDOW_ALWAYS_RUN: on" else "[A] FLAG_WINDOW_ALWAYS_RUN: off", 10, 240, 10, if rl.is_window_state(always_run_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(vsync_flag) then "[V] FLAG_VSYNC_HINT: on" else "[V] FLAG_VSYNC_HINT: off", 10, 260, 10, if rl.is_window_state(vsync_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(borderless_flag) then "[B] FLAG_BORDERLESS_WINDOWED_MODE: on" else "[B] FLAG_BORDERLESS_WINDOWED_MODE: off", 10, 280, 10, if rl.is_window_state(borderless_flag) then rl.LIME else rl.MAROON)

        rl.draw_text("Following flags can only be set before window creation:", 10, 320, 10, rl.GRAY)
        rl.draw_text(if rl.is_window_state(highdpi_flag) then "FLAG_WINDOW_HIGHDPI: on" else "FLAG_WINDOW_HIGHDPI: off", 10, 340, 10, if rl.is_window_state(highdpi_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(transparent_flag) then "FLAG_WINDOW_TRANSPARENT: on" else "FLAG_WINDOW_TRANSPARENT: off", 10, 360, 10, if rl.is_window_state(transparent_flag) then rl.LIME else rl.MAROON)
        rl.draw_text(if rl.is_window_state(msaa_flag) then "FLAG_MSAA_4X_HINT: on" else "FLAG_MSAA_4X_HINT: off", 10, 380, 10, if rl.is_window_state(msaa_flag) then rl.LIME else rl.MAROON)
