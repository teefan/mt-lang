...
def main() -> int:
    ...
    while not rl.window_should_close():
        ...
        if rl.is_mouse_button_pressed(rl.MouseButton.MOUSE_BUTTON_RIGHT) or (rl.is_key_down(rl.KeyboardKey.KEY_LEFT_CONTROL) and rl.is_mouse_button_down(rl.MouseButton.MOUSE_BUTTON_RIGHT)):
            if ball_count < max_balls:
                ...
                balls_view[new_index] = ball_at(
                    ...
                    20.0 + float<-rl.get_random_value(0, 30),
                    rl.Color(
        ...
        rl.draw_text(rl.text_format_int("BALL COUNT: %d", ball_count), 10, rl.get_screen_height() - 70, 20, rl.BLACK)
        rl.draw_text(rl.text_format_float("GRAVITY: %.2f", gravity), 10, rl.get_screen_height() - 40, 20, rl.BLACK)
