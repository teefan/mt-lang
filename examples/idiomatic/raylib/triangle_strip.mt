...
def main() -> i32:
    ...
    let center = rl.Vector2(x = cast[f32](screen_width) / 2.0 - 125.0, y = cast[f32](screen_height) / 2.0)
    ...
    while not rl.window_should_close():
        ...
        let angle_step = 360.0 / cast[f32](point_count) * math.deg2rad

        for index in range(0, point_count):
            let point_index = index * 2
            let inner_angle = cast[f32](index) * angle_step
