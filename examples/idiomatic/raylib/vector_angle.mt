...
def main() -> int:
    ...
    let origin = rl.Vector2(x = float<-screen_width / 2.0, y = float<-screen_height / 2.0)
    ...
    while not rl.window_should_close():
        ...
        rl.draw_text("v0", int<-origin.x, int<-origin.y, 10, rl.DARKGRAY)
        if angle_mode == 0 and origin.subtract(first_vector).y > 0.0:
            rl.draw_text("v1", int<-first_vector.x, int<-first_vector.y - 10, 10, rl.DARKGRAY)
        elif angle_mode == 0 and origin.subtract(first_vector).y < 0.0:
            rl.draw_text("v1", int<-first_vector.x, int<-first_vector.y, 10, rl.DARKGRAY)
        elif angle_mode == 1:
            rl.draw_text("v1", int<-origin.x + 40, int<-origin.y, 10, rl.DARKGRAY)

        rl.draw_text("v2", int<-second_vector.x - 10, int<-second_vector.y - 10, 10, rl.DARKGRAY)
        rl.draw_text("Press SPACE to change MODE", 460, 10, 20, rl.DARKGRAY)
        rl.draw_text(rl.text_format_float("ANGLE: %2.2f", angle), 10, 70, 20, rl.LIME)
