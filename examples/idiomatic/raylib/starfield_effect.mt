...
def main() -> int:
    ...
    while not rl.window_should_close():
        ...
        for index in 0..star_count:
            ...
            if stars[index].z < 0.0 or screen_positions[index].x < 0.0 or screen_positions[index].y < 0.0 or screen_positions[index].x > float<-screen_width or screen_positions[index].y > float<-screen_height:
                stars[index].x = float<-rl.get_random_value(-screen_width / 2, screen_width / 2)
                stars[index].y = float<-rl.get_random_value(-screen_height / 2, screen_height / 2)
                stars[index].z = 1.0
        ...
        rl.draw_text(rl.text_format_float("[MOUSE WHEEL] Current Speed: %.0f", 9.0 * speed / 2.0), 10, 40, 20, rl.RAYWHITE)
