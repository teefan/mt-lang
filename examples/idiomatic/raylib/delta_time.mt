...
def main() -> i32:
    ...
    while not rl.window_should_close():
        ...
        if current_fps <= 0:
            rl.draw_text(rl.text_format_i32("FPS: unlimited (%i)", rl.get_fps()), 10, 10, 20, rl.DARKGRAY)
        else:
            rl.draw_text(rl.text_format_i32_i32("FPS: %i (target: %i)", rl.get_fps(), current_fps), 10, 10, 20, rl.DARKGRAY)

        rl.draw_text(rl.text_format_f32("Frame time: %.2f ms", rl.get_frame_time()), 10, 30, 20, rl.DARKGRAY)
