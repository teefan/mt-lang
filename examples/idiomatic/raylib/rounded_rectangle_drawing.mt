...
def main() -> i32:
    ...
    while not rl.window_should_close():
        let rec = rl.Rectangle(
            x = (cast[f32](rl.get_screen_width()) - width - 250.0) / 2.0,
            y = (cast[f32](rl.get_screen_height()) - height) / 2.0,
            width = width,
            height = height,
        )
        ...
        if draw_rounded_rect:
            rl.draw_rectangle_rounded(rec, roundness, cast[i32](segments), rl.fade(rl.MAROON, 0.2))
        if draw_rounded_lines:
            rl.draw_rectangle_rounded_lines_ex(rec, roundness, cast[i32](segments), line_thick, rl.fade(rl.MAROON, 0.4))

        gui.slider_bar(rl.Rectangle(x = 640.0, y = 40.0, width = 105.0, height = 20.0), "Width", rl.text_format_f32("%.2f", width), inout width, 0.0, cast[f32](rl.get_screen_width()) - 300.0)
        gui.slider_bar(rl.Rectangle(x = 640.0, y = 70.0, width = 105.0, height = 20.0), "Height", rl.text_format_f32("%.2f", height), inout height, 0.0, cast[f32](rl.get_screen_height()) - 50.0)
