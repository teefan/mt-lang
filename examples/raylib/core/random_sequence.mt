...
function generate_random_color_rect_sequence(rect_count: int, rect_width: float, screen_width: float, screen_height: float) -> vec.Vec[ColorRect]:
...
function main() -> int:
    ...
    var rectangles = generate_random_color_rect_sequence(rect_count, rect_size, float<-SCREEN_WIDTH, 0.75 * float<-SCREEN_HEIGHT)
    ...
    while not rl.window_should_close():
        ...
        if rl.is_key_pressed(rl.KeyboardKey.KEY_UP):
            ...
            rectangles.release()
            rectangles = generate_random_color_rect_sequence(rect_count, rect_size, float<-SCREEN_WIDTH, 0.75 * float<-SCREEN_HEIGHT)
        ...
        rl.draw_text("Press DOWN to remove a rectangle and generate a new sequence", 10, SCREEN_HEIGHT - 32, 20, rl.BLACK)
