...
def main() -> i32:
    ...
    let space_char = cast[char](32)
    let null_char = cast[char](0)
    let newline_char = cast[char](10)
        ...
        unsafe:
            ...
            while j <= line_length:
                ...
                if current == space_char or current == null_char:
                    ...
                    if rl.MeasureText(cast[cstr](line + last_wrap_start), font_size) > wrap_width:
    ...
    var scroll_bar = rl.Rectangle(
        x = cast[f32](screen_width) - 5.0,
        y = cast[f32](text_top),
        width = 5.0,
        height = if text_height > screen_height then cast[f32](screen_height) * 100.0 / cast[f32](scroll_range) else cast[f32](screen_height - text_top),
    )
    ...
    while not rl.WindowShouldClose():
        ...
        cam.target.y -= scroll * cast[f32](font_size) * 1.5

        if cam.target.y < 0.0:
            cam.target.y = 0.0

        let max_target_y = if text_height > screen_height then cast[f32](text_height - screen_height + text_top) else 0.0
        if cam.target.y > max_target_y:
            cam.target.y = max_target_y

        let scroll_ratio: f32 = if text_height > screen_height then rm.clamp((cam.target.y - cast[f32](text_top)) / cast[f32](scroll_range), 0.0, 1.0) else 0.0
        ...
        while draw_index < line_count:
            unsafe:
                ...
                draw_y += cast[i32](size.y) + line_gap
            draw_index += 1
        rl.EndMode2D()
