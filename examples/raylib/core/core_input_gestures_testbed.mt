...
def main() -> int:
    ...
    var previous_gesture: int = rl.Gesture.GESTURE_NONE
    var log_mode = 1
    ...
    while not rl.WindowShouldClose():
        ...
        rl.DrawText(c"*", int<-message_position.x + 5, int<-message_position.y + 5, 10, rl.BLACK)
        rl.DrawText(mobile_message, int<-message_position.x + 15, int<-message_position.y + 5, 10, rl.BLACK)
        rl.DrawText(c"*", int<-message_position.x + 5, int<-message_position.y + 35, 10, rl.BLACK)
        rl.DrawText(desktop_message, int<-message_position.x + 15, int<-message_position.y + 35, 10, rl.BLACK)
        ...
        rl.DrawText(c"Last gesture", int<-last_gesture_position.x + 33, int<-last_gesture_position.y - 47, 20, rl.BLACK)
        rl.DrawText(c"Swipe         Tap       Pinch  Touch", int<-last_gesture_position.x + 17, int<-last_gesture_position.y - 18, 10, rl.BLACK)
        rl.DrawRectangle(int<-last_gesture_position.x + 20, int<-last_gesture_position.y, 20, 20, swipe_up_color)
        rl.DrawRectangle(int<-last_gesture_position.x, int<-last_gesture_position.y + 20, 20, 20, swipe_left_color)
        rl.DrawRectangle(int<-last_gesture_position.x + 40, int<-last_gesture_position.y + 20, 20, 20, swipe_right_color)
        rl.DrawRectangle(int<-last_gesture_position.x + 20, int<-last_gesture_position.y + 40, 20, 20, swipe_down_color)
        rl.DrawCircle(int<-last_gesture_position.x + 80, int<-last_gesture_position.y + 16, 10.0, tap_color)
        rl.DrawRing(rl.Vector2(x = last_gesture_position.x + 103.0, y = last_gesture_position.y + 16.0), 6.0, 11.0, 0.0, 360.0, 0, drag_color)
        rl.DrawCircle(int<-last_gesture_position.x + 80, int<-last_gesture_position.y + 43, 10.0, doubletap_color)
        rl.DrawCircle(int<-last_gesture_position.x + 103, int<-last_gesture_position.y + 43, 10.0, doubletap_color)
        ...
        for touch_indicator in 0..4:
            rl.DrawCircle(int<-last_gesture_position.x + 180, int<-last_gesture_position.y + 7 + touch_indicator * 15, 5.0, if touch_count <= touch_indicator: rl.LIGHTGRAY else: current_gesture_color)

        rl.DrawText(c"Log", int<-gesture_log_position.x, int<-gesture_log_position.y, 20, rl.BLACK)
        var log_row = 0
        var log_index = gesture_log_index % gesture_log_size
        while log_row < gesture_log_size:
            rl.DrawText(gesture_log[log_index], int<-gesture_log_position.x, int<-gesture_log_position.y + 410 - log_row * 20, 20, if log_row == 0: current_gesture_color else: rl.LIGHTGRAY)
            log_row += 1
        ...
        rl.DrawText(c"Hide", int<-log_button1.x + 7, int<-log_button1.y + 3, 10, rl.WHITE)
        rl.DrawText(c"Repeat", int<-log_button1.x + 7, int<-log_button1.y + 13, 10, rl.WHITE)
        rl.DrawRectangleRec(log_button2, log_button2_color)
        rl.DrawText(c"Hide", int<-log_button1.x + 62, int<-log_button1.y + 3, 10, rl.WHITE)
        rl.DrawText(c"Hold", int<-log_button1.x + 62, int<-log_button1.y + 13, 10, rl.WHITE)

        rl.DrawText(c"Angle", int<-protractor_position.x + 55, int<-protractor_position.y + 76, 10, rl.BLACK)
        ...
        rl.DrawText(angle_string_trim, int<-protractor_position.x + 55, int<-protractor_position.y + 92, 20, current_gesture_color)
        ...
        rl.DrawLineEx(rl.Vector2(x = protractor_position.x - 80.0, y = protractor_position.y + 45.0), rl.Vector2(x = protractor_position.x + 80.0, y = protractor_position.y - 45.0), 3.0, rl.GREEN)
        rl.DrawText(c"0", int<-protractor_position.x + 96, int<-protractor_position.y - 9, 20, rl.BLACK)
        rl.DrawText(c"30", int<-protractor_position.x + 74, int<-protractor_position.y - 68, 20, rl.BLACK)
        rl.DrawText(c"90", int<-protractor_position.x - 11, int<-protractor_position.y - 110, 20, rl.BLACK)
        rl.DrawText(c"150", int<-protractor_position.x - 100, int<-protractor_position.y - 68, 20, rl.BLACK)
        rl.DrawText(c"180", int<-protractor_position.x - 124, int<-protractor_position.y - 9, 20, rl.BLACK)
        rl.DrawText(c"210", int<-protractor_position.x - 100, int<-protractor_position.y + 50, 20, rl.BLACK)
        rl.DrawText(c"270", int<-protractor_position.x - 18, int<-protractor_position.y + 92, 20, rl.BLACK)
        rl.DrawText(c"330", int<-protractor_position.x + 72, int<-protractor_position.y + 50, 20, rl.BLACK)
