...
def main() -> i32:
    ...
    while not rl.WindowShouldClose():
        ...
        var player_color = rl.RED
        if release_action:
            player_color = rl.BLUE
        rl.DrawRectangleV(position, size, player_color)

        if action_set:
            rl.DrawText(cursor_set_text, 10, 10, 20, rl.WHITE)
        else:
            rl.DrawText(default_set_text, 10, 10, 20, rl.WHITE)
