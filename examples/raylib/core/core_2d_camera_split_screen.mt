...
def draw_grid() -> void:
    ...
    while column < screen_width / player_size + 1:
        let x: f32 = 1.0 * (player_size * column)
        rl.DrawLineV(rl.Vector2(x = x, y = 0.0), rl.Vector2(x = x, y = 1.0 * screen_height), rl.LIGHTGRAY)
        column += 1

    var row = 0
    while row < screen_height / player_size + 1:
        let y: f32 = 1.0 * (player_size * row)
        rl.DrawLineV(rl.Vector2(x = 0.0, y = y), rl.Vector2(x = 1.0 * screen_width, y = y), rl.LIGHTGRAY)
        row += 1
...
def main() -> i32:
    ...
    var player1 = rl.Rectangle(x = 200.0, y = 200.0, width = 1.0 * player_size, height = 1.0 * player_size)
    var player2 = rl.Rectangle(x = 250.0, y = 200.0, width = 1.0 * player_size, height = 1.0 * player_size)
    ...
    let split_screen_rect = rl.Rectangle(
        x = 0.0,
        y = 0.0,
        width = 1.0 * screen_camera1.texture.width,
        height = -1.0 * screen_camera1.texture.height,
    )
