...
function main() -> int:
    ...
    let words = array[str, WORD_COUNT](
        "raylib",
        "is",
        "a",
        "simple",
    ...
    while not rl.window_should_close():
        ...
        let text_pos = rl.Vector2(
            x = text_container_rect.x + rm.lerp(
                ...
                alignment_amount(h_align)
            ),
            y = text_container_rect.y + rm.lerp(
                0.0,
                text_container_rect.height - text_size.y,
                alignment_amount(v_align)
            )
        )
        ...
        rl.draw_text(
            rl.text_format(
                "Alignment: Horizontal = %s, Vertical = %s",
                text_align_name_h[h_align],
                text_align_name_v[v_align]
            ),
            20,
            40,
            20,
