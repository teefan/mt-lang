    ...
    while not rl.WindowShouldClose():
        if rl.IsKeyPressed(rl.KeyboardKey.KEY_LEFT):
            if h_align > TextAlignment.TEXT_ALIGN_LEFT:
                h_align = TextAlignment<-(int<-h_align - 1)
        ...
        if rl.IsKeyPressed(rl.KeyboardKey.KEY_UP):
            if v_align > TextAlignment.TEXT_ALIGN_TOP:
                v_align = TextAlignment<-(int<-v_align - 1)
        ...
        if word_count > 0:
            word_index = int<-rl.GetTime() % word_count
        else:
            word_index = 0

        unsafe:
            let current_word = cstr<-read(words + word_index)
            let text_size = rl.MeasureTextEx(font, current_word, float<-font_size, float<-font_size * 0.1)
            let text_pos = rl.Vector2(
                x = text_container_rect.x + rm.lerp(0.0, text_container_rect.width - text_size.x, float<-h_align * 0.5),
                y = text_container_rect.y + rm.lerp(0.0, text_container_rect.height - text_size.y, float<-v_align * 0.5),
            )
            ...
            rl.DrawText(
                rl.TextFormat(align_format, text_align_name_h[int<-h_align], text_align_name_v[int<-v_align]),
                20,
                40,
                20,
                rl.LIGHTGRAY,
            ...
            rl.DrawTextEx(font, current_word, text_pos, float<-font_size, float<-font_size * 0.1, rl.RAYWHITE)
