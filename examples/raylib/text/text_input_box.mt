...
def main() -> i32:
    ...
    while not rl.WindowShouldClose():
        ...
        if mouse_on_text:
            ...
            while key > 0:
                if key >= 32 and key <= 125 and letter_count < max_input_chars:
                    name[letter_count] = cast[u8](key)
                    name[letter_count + 1] = cast[u8](0)
                    letter_count += 1
        ...
        if mouse_on_text:
            rl.DrawRectangleLines(cast[i32](text_box.x), cast[i32](text_box.y), cast[i32](text_box.width), cast[i32](text_box.height), rl.RED)
        else:
            rl.DrawRectangleLines(cast[i32](text_box.x), cast[i32](text_box.y), cast[i32](text_box.width), cast[i32](text_box.height), rl.DARKGRAY)

        rl.DrawText(name_text, cast[i32](text_box.x) + 5, cast[i32](text_box.y) + 8, 40, rl.MAROON)
        ...
        if mouse_on_text:
            if letter_count < max_input_chars:
                if ((frames_counter / 20) % 2) == 0:
                    rl.DrawText(underscore_text, cast[i32](text_box.x) + 8 + rl.MeasureText(name_text, 40), cast[i32](text_box.y) + 12, 40, rl.MAROON)
            else:
