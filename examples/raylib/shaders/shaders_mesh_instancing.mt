...
def set_transform(transforms: ptr[rl.Matrix], index: int, transform: rl.Matrix) -> void:
    unsafe:
        read(transforms + ptr_uint<-index) = transform


def set_material_color(material: ptr[rl.Material], map_index: int, color: rl.Color) -> void:
    unsafe:
...
def main() -> int:
    ...
    for index in 0..max_instances:
        ...
        let axis = rl.Vector3(
            x = float<-rl.GetRandomValue(0, 360),
            y = float<-rl.GetRandomValue(0, 360),
            z = float<-rl.GetRandomValue(0, 360),
        ).normalize()
        let angle = float<-rl.GetRandomValue(0, 180) * rm.deg2rad
    ...
    unsafe:
        shader.locs[int<-rl.ShaderLocationIndex.SHADER_LOC_MATRIX_MVP] = mvp_loc
    ...
    lights.CreateLight(int<-lights.LightType.LIGHT_DIRECTIONAL, rl.Vector3(x = 50.0, y = 50.0, z = 0.0), rm.Vector3.zero(), rl.WHITE, shader)

    var mat_instances = rl.LoadMaterialDefault()
    mat_instances.shader = shader
    set_material_color(ptr_of(mat_instances), int<-rl.MaterialMapIndex.MATERIAL_MAP_ALBEDO, rl.RED)

    var mat_default = rl.LoadMaterialDefault()
    set_material_color(ptr_of(mat_default), int<-rl.MaterialMapIndex.MATERIAL_MAP_ALBEDO, rl.BLUE)
