...
function main() -> int:
    ...
    while not rl.window_should_close():
        ...
        let camera_direction_vector = rm.vector3_scale(
            rm.vector3_normalize(rm.vector3_subtract(camera.target, camera.position)),
            cam_dist
        )
        let camera_direction = array[float, 3](
            camera_direction_vector.x,
            camera_direction_vector.y,
            camera_direction_vector.z
        )

        rl.set_shader_value(
            raymarch_shader,
            ray_locations.cam_pos,
            camera_position,
            int<-rl.ShaderUniformDataType.SHADER_UNIFORM_VEC3
        )
        rl.set_shader_value(
            raymarch_shader,
            ray_locations.cam_dir,
            camera_direction,
            int<-rl.ShaderUniformDataType.SHADER_UNIFORM_VEC3
        )
