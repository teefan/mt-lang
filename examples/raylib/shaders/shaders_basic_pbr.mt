...
def set_shader_int(shader: rl.Shader, location: i32, value: i32) -> void:
    var storage = value
    rl.SetShaderValue(shader, location, i32_ptr_to_void(raw(addr(storage))), rl.ShaderUniformDataType.SHADER_UNIFORM_INT)
...
def update_light(shader: rl.Shader, light: Light) -> void:
    ...
    rl.SetShaderValue(shader, light.enabled_loc, i32_ptr_to_void(raw(addr(enabled))), rl.ShaderUniformDataType.SHADER_UNIFORM_INT)
    rl.SetShaderValue(shader, light.type_loc, i32_ptr_to_void(raw(addr(type_value))), rl.ShaderUniformDataType.SHADER_UNIFORM_INT)
    rl.SetShaderValue(shader, light.position_loc, f32_ptr_to_void(raw(addr(position[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC3)
    rl.SetShaderValue(shader, light.target_loc, f32_ptr_to_void(raw(addr(target[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC3)
    rl.SetShaderValue(shader, light.color_loc, f32_ptr_to_void(raw(addr(color[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC4)
    rl.SetShaderValue(shader, light.intensity_loc, f32_ptr_to_void(raw(addr(intensity))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
...
def main() -> i32:
    ...
    rl.SetShaderValue(shader, ambient_color_loc, f32_ptr_to_void(raw(addr(ambient_color[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC3)
    rl.SetShaderValue(shader, ambient_intensity_loc, f32_ptr_to_void(raw(addr(ambient_intensity))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
    ...
    while not rl.WindowShouldClose():
        ...
        rl.SetShaderValue(shader, texture_tiling_loc, f32_ptr_to_void(raw(addr(floor_texture_tiling[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC2)
        rl.SetShaderValue(shader, emissive_color_loc, f32_ptr_to_void(raw(addr(floor_emissive[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC4)
        rl.SetShaderValue(shader, metallic_value_loc, f32_ptr_to_void(raw(addr(floor_metallic_value))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
        rl.SetShaderValue(shader, roughness_value_loc, f32_ptr_to_void(raw(addr(floor_roughness_value))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
        rl.DrawModel(floor, rm.Vector3.zero(), 5.0, rl.WHITE)

        rl.SetShaderValue(shader, texture_tiling_loc, f32_ptr_to_void(raw(addr(car_texture_tiling[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC2)
        rl.SetShaderValue(shader, emissive_color_loc, f32_ptr_to_void(raw(addr(car_emissive[0]))), rl.ShaderUniformDataType.SHADER_UNIFORM_VEC4)
        rl.SetShaderValue(shader, emissive_intensity_loc, f32_ptr_to_void(raw(addr(emissive_intensity))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
        rl.SetShaderValue(shader, metallic_value_loc, f32_ptr_to_void(raw(addr(car_metallic_value))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
        rl.SetShaderValue(shader, roughness_value_loc, f32_ptr_to_void(raw(addr(car_roughness_value))), rl.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
