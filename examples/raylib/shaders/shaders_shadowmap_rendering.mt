...
def main() -> i32:
    ...
    set_model_shader(raw(addr(cube)), shadow_shader)

    var robot = rl.LoadModel(robot_model_path)
    defer rl.UnloadModel(robot)
    set_all_model_shaders(raw(addr(robot)), shadow_shader)

    var anim_count = 0
    let anims = rl.LoadModelAnimations(robot_model_path, raw(addr(anim_count)))
