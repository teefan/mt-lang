...
function main() -> int:
    ...
    unsafe: shader.locs[int<-rl.ShaderLocationIndex.SHADER_LOC_MATRIX_MODEL] = rl.get_shader_location(shader, "matModel")
    ...
    create_light(0, int<-LightType.LIGHT_POINT, rl.Vector3(x = 0.0, y = 2.0, z = 6.0), rl.Vector3(x = 0.0, y = 0.0, z = 0.0), rl.WHITE, shader)
    ...
    while not rl.window_should_close():
        ...
        rl.draw_text(rl.text_format("Use KEY_UP/KEY_DOWN to change fog density [%.2f]", fog_density), 10, 10, 20, rl.RAYWHITE)
        rl.end_drawing()
