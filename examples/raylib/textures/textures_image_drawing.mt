...
def main() -> i32:
    ...
    rl.ImageCrop(raw(addr(cat)), rl.Rectangle(x = 100.0, y = 10.0, width = 280.0, height = 380.0))
    rl.ImageFlipHorizontal(raw(addr(cat)))
    rl.ImageResize(raw(addr(cat)), 150, 200)
    ...
    rl.ImageDrawRectangle(raw(addr(parrots)), 5, 20, 10, 10, rl.RAYWHITE)
