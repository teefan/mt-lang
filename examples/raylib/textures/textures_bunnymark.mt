    ...
    while not rl.WindowShouldClose():
        if rl.IsMouseButtonDown(rl.MouseButton.MOUSE_BUTTON_LEFT):
            for index in range(0, 100):
                ...
                if bunnies_count < max_bunnies:
                    ...
                    bunnies_view[bunnies_count].speed.x = f32<-rl.GetRandomValue(-250, 250)
                    bunnies_view[bunnies_count].speed.y = f32<-rl.GetRandomValue(-250, 250)
                    bunnies_view[bunnies_count].color = rl.Color(
                        r = u8<-rl.GetRandomValue(50, 240),
                        g = u8<-rl.GetRandomValue(80, 240),
                        b = u8<-rl.GetRandomValue(100, 240),
                        a = 255,
                    )
                    bunnies_count += 1
        ...
        if not paused:
            ...
            for index in range(0, bunnies_count):
                ...
                if bunnies_view[index].position.x + f32<-tex_bunny.width / 2.0 > f32<-rl.GetScreenWidth() or bunnies_view[index].position.x + f32<-tex_bunny.width / 2.0 < 0.0:
                    bunnies_view[index].speed.x *= -1.0
                if bunnies_view[index].position.y + f32<-tex_bunny.height / 2.0 > f32<-rl.GetScreenHeight() or bunnies_view[index].position.y + f32<-tex_bunny.height / 2.0 - 40.0 < 0.0:
