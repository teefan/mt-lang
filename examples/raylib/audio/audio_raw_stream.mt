...
def main() -> int:
    ...
    var buffer = zero[array[float, 4096]]
    ...
    while not rl.WindowShouldClose():
        ...
        if rl.IsAudioStreamProcessed(stream):
            for index in 0..buffer_size:
                ...
                let phase = 2.0 * rl.PI * float<-sine_index / float<-wavelength
        ...
        let window_start = int<-((rl.GetTime() - sine_start_time) * double<-sample_rate)
        let window_size = int<-(0.1 * float<-sample_rate)
        let wavelength = sample_rate / sine_frequency
        ...
        for index in 0..screen_width:
            ...
            let start_pos = rl.Vector2(
                x = float<-index,
                y = 250.0 + 50.0 * rm.sin(2.0 * rl.PI * float<-t0 / float<-wavelength),
            )
            let end_pos = rl.Vector2(
                x = float<-(index + 1),
                y = 250.0 + 50.0 * rm.sin(2.0 * rl.PI * float<-t1 / float<-wavelength),
            )
