...
const lpf_cutoff: float = 70.0 / 44100.0
const lpf_k: float = lpf_cutoff / (lpf_cutoff + 0.1591549431)

var delay_buffer: ptr[float]? = null
var delay_buffer_size: uint = 0
var delay_read_index: uint = 2
...
def audio_process_effect_delay(buffer: ptr[void], frames: uint) -> void:
    ...
    unsafe:
        let delay_samples = ptr[float]<-delay_buffer
        var index = 0

        while index < sample_count:
            let left_delay = read(delay_samples + int<-delay_read_index)
            delay_read_index += 1
            let right_delay = read(delay_samples + int<-delay_read_index)
            delay_read_index += 1
            ...
            read(delay_samples + int<-delay_write_index) = left_value
            delay_write_index += 1
            read(delay_samples + int<-delay_write_index) = right_value
            delay_write_index += 1
...
def main() -> int:
    ...
    while not rl.WindowShouldClose():
        ...
        rl.DrawRectangle(200, 180, int<-(time_played * 400.0), 12, rl.MAROON)
