...
def main() -> int:
    ...
    while not rl.WindowShouldClose():
        ...
        let half_blocks = float<-num_blocks / 2.0
            ...
            for y in 0..num_blocks:
                for z in 0..num_blocks:
                    ...
                    let cube_pos = rl.Vector3(
                        x = (float<-x - half_blocks) * (scale * 3.0) + scatter,
                        y = (float<-y - half_blocks) * (scale * 2.0) + scatter,
                        z = (float<-z - half_blocks) * (scale * 3.0) + scatter,
                    )
                    let cube_color = rl.ColorFromHSV(float<-(((x + y + z) * 18) % 360), 0.75, 0.9)
