...
def draw_sphere_basic(color: rl.Color) -> void:
    ...
    for i in 0..rings + 2:
        let latitude0 = rm.deg2rad * (270.0 + (180.0 / float<-(rings + 1)) * float<-i)
        let latitude1 = rm.deg2rad * (270.0 + (180.0 / float<-(rings + 1)) * float<-(i + 1))
        ...
        for j in 0..slices:
            let longitude0 = rm.deg2rad * (float<-j * 360.0 / float<-slices)
            let longitude1 = rm.deg2rad * (float<-(j + 1) * 360.0 / float<-slices)
