...
def main() -> int:
    ...
    while not rl.WindowShouldClose():
        ...
        let tangent: float = rm.clamp(math.tanf(angle_rad), -10.0, 10.0)
        let cotangent: float = if math.fabsf(tangent) > 0.001: rm.clamp(1.0 / tangent, -radius, radius) else: 0.0
        ...
        rl.DrawSplineLinear(ptr_of(sine_points[0]), wave_points, 1.0, rl.RED)
        ...
        rl.DrawSplineLinear(ptr_of(cos_points[0]), wave_points, 1.0, rl.BLUE)
        ...
        gui.GuiToggle(gui.Rectangle(x = 640.0, y = 70.0, width = 120.0, height = 20.0), pause_text, ptr_of(pause))
        gui.GuiSetStyle(gui.GuiControl.LABEL, gui.GuiControlProperty.TEXT_COLOR_NORMAL, rl.ColorToInt(rl.LIME))
        gui.GuiSliderBar(gui.Rectangle(x = 640.0, y = 40.0, width = 120.0, height = 20.0), angle_text, rl.TextFormat(c"%.0f deg", angle), ptr_of(angle), 0.0, 360.0)
