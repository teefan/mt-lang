...
function main() -> int:
    ...
    var values: array[float, MAX_PIE_SLICES] = array[float, MAX_PIE_SLICES](300.0, 100.0, 450.0, 350.0, 600.0, 380.0, 750.0, 0.0, 0.0, 0.0)
    ...
    let panel_width = 270
    let panel_margin = 5
    let panel_pos = rl.Vector2(x = float<-SCREEN_WIDTH - float<-panel_margin - float<-panel_width, y = float<-panel_margin)
    let panel_rect = rl.Rectangle(x = panel_pos.x, y = panel_pos.y, width = float<-panel_width, height = float<-SCREEN_HEIGHT - (2.0 * float<-panel_margin))
    ...
    while not rl.window_should_close():
        ...
        gui.spinner(rl.Rectangle(x = panel_pos.x + 95.0, y = panel_pos.y + 12.0, width = 125.0, height = 25.0), "Slices ", slice_count, 1, MAX_PIE_SLICES, false)
        gui.check_box(rl.Rectangle(x = panel_pos.x + 20.0, y = panel_pos.y + 52.0, width = 20.0, height = 20.0), "Show Values", show_values)
        gui.check_box(rl.Rectangle(x = panel_pos.x + 20.0, y = panel_pos.y + 82.0, width = 20.0, height = 20.0), "Show Percentages", show_percentages)
        gui.check_box(rl.Rectangle(x = panel_pos.x + 20.0, y = panel_pos.y + 112.0, width = 20.0, height = 20.0), "Make Donut", show_donut)

        if not show_donut:
            gui.disable()
        gui.slider_bar(rl.Rectangle(x = panel_pos.x + 80.0, y = panel_pos.y + 142.0, width = panel_rect.width - 100.0, height = 30.0), "Inner Radius", "", donut_inner_radius, 5.0, radius - 10.0)
        gui.enable()

        gui.line(rl.Rectangle(x = panel_pos.x + 10.0, y = panel_pos.y + 182.0, width = panel_rect.width - 20.0, height = 1.0), "")
