...
function main() -> int:
    ...
    while not rl.window_should_close():
        ...
        rl.draw_line_ex(rl.Vector2(x = float<-center.x, y = float<-limit_min.y), rl.Vector2(x = float<-cotangent_point.x, y = float<-limit_min.y), 2.0, rl.ORANGE)
        ...
        rl.draw_line_ex(rl.Vector2(x = float<-center.x, y = float<-limit_min.y), rl.Vector2(x = float<-center.x, y = float<-limit_max.y), 1.0, rl.GRAY)
        rl.draw_line_ex(rl.Vector2(x = float<-limit_min.x, y = float<-center.y), rl.Vector2(x = float<-limit_max.x, y = float<-center.y), 1.0, rl.GRAY)

        rl.draw_line_ex(rl.Vector2(x = float<-graph_bounds.x, y = float<-graph_bounds.y), rl.Vector2(x = float<-graph_bounds.x, y = float<-(graph_bounds.y + graph_bounds.height)), 2.0, rl.GRAY)
        rl.draw_line_ex(rl.Vector2(x = float<-(graph_bounds.x + graph_bounds.width), y = float<-graph_bounds.y), rl.Vector2(x = float<-(graph_bounds.x + graph_bounds.width), y = float<-(graph_bounds.y + graph_bounds.height)), 2.0, rl.GRAY)
        rl.draw_line_ex(rl.Vector2(x = float<-graph_bounds.x, y = float<-(graph_bounds.y + half_graph_height)), rl.Vector2(x = float<-(graph_bounds.x + graph_bounds.width), y = float<-(graph_bounds.y + half_graph_height)), 2.0, rl.GRAY)
        ...
        rl.draw_text("360", int<-(graph_bounds.x + graph_bounds.width) - 8, int<-(graph_bounds.y + graph_bounds.height) + 4, 6, rl.GRAY)

        rl.draw_line_ex(rl.Vector2(x = float<-center.x, y = float<-center.y), rl.Vector2(x = float<-center.x, y = float<-point.y), 2.0, rl.RED)
        rl.draw_line_dashed(rl.Vector2(x = float<-point.x, y = float<-center.y), rl.Vector2(x = float<-point.x, y = float<-point.y), 10, 4, rl.RED)
        rl.draw_text(text.cstr_as_str(rl.text_format("Sine %.2f", sin_rad)), 640, 190, 6, rl.RED)
        rl.draw_circle_v(rl.Vector2(x = float<-(graph_bounds.x + (angle / 360.0) * graph_bounds.width), y = float<-(graph_bounds.y + ((-sin_rad + 1.0) * half_graph_height))), 4.0, rl.RED)
        rl.draw_spline_linear_ptr(ptr_of(sine_points[0]), WAVE_POINTS, 1.0, rl.RED)

        rl.draw_line_ex(rl.Vector2(x = float<-center.x, y = float<-center.y), rl.Vector2(x = float<-point.x, y = float<-center.y), 2.0, rl.BLUE)
        rl.draw_line_dashed(rl.Vector2(x = float<-center.x, y = float<-point.y), rl.Vector2(x = float<-point.x, y = float<-point.y), 10, 4, rl.BLUE)
        rl.draw_text(text.cstr_as_str(rl.text_format("Cosine %.2f", cos_rad)), 640, 210, 6, rl.BLUE)
        rl.draw_circle_v(rl.Vector2(x = float<-(graph_bounds.x + (angle / 360.0) * graph_bounds.width), y = float<-(graph_bounds.y + ((-cos_rad + 1.0) * half_graph_height))), 4.0, rl.BLUE)
        rl.draw_spline_linear_ptr(ptr_of(cos_points[0]), WAVE_POINTS, 1.0, rl.BLUE)

        rl.draw_line_ex(rl.Vector2(x = float<-limit_max.x, y = float<-center.y), rl.Vector2(x = float<-limit_max.x, y = float<-tangent_point.y), 2.0, rl.PURPLE)
        ...
        rl.draw_text(text.cstr_as_str(rl.text_format("Supplementary %.0f deg", supplementary)), 640, 130, 6, rl.DARKBLUE)
        ...
        gui.slider_bar(rl.Rectangle(x = 640.0, y = 40.0, width = 120.0, height = 20.0), "Angle", text.cstr_as_str(rl.text_format("%.0f deg", angle)), angle, 0.0, 360.0)
