...
def main() -> int:
    ...
    while not rl.WindowShouldClose():
        ...
        rl.DrawText(v0_label, int<-v0.x, int<-v0.y, 10, rl.DARKGRAY)

        if angle_mode == 0 and v0.subtract(v1).y > 0.0:
            rl.DrawText(v1_label, int<-v1.x, int<-v1.y - 10, 10, rl.DARKGRAY)
        elif angle_mode == 0 and v0.subtract(v1).y < 0.0:
            rl.DrawText(v1_label, int<-v1.x, int<-v1.y, 10, rl.DARKGRAY)

        if angle_mode == 1:
            rl.DrawText(v1_label, int<-v0.x + 40, int<-v0.y, 10, rl.DARKGRAY)

        rl.DrawText(v2_label, int<-v2.x - 10, int<-v2.y - 10, 10, rl.DARKGRAY)
