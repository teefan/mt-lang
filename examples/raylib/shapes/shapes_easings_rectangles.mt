    ...
    while not rl.WindowShouldClose():
        if state == 0:
            ...
            for index in range(0, rec_count):
                recs[index].height = ease_circ_out(cast[f32](frames_counter), cast[f32](recs_height), -cast[f32](recs_height), cast[f32](play_time_in_frames))
                recs[index].width = ease_circ_out(cast[f32](frames_counter), cast[f32](recs_width), -cast[f32](recs_width), cast[f32](play_time_in_frames))
                ...
                rotation = ease_linear_in(cast[f32](frames_counter), 0.0, 360.0, cast[f32](play_time_in_frames))
