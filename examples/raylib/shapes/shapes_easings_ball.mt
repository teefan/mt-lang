...
def main() -> int:
    ...
    while not rl.WindowShouldClose():
        if state == 0:
            frames_counter += 1
            ball_position_x = int<-ease_elastic_out(float<-frames_counter, -100.0, ball_position_delta, 120.0)
            ...
            frames_counter += 1
            ball_radius = int<-ease_elastic_in(float<-frames_counter, 20.0, 500.0, 200.0)
            ...
            frames_counter += 1
            ball_alpha = ease_cubic_out(float<-frames_counter, 0.0, 1.0, 200.0)
        ...
        rl.DrawCircle(ball_position_x, 200, float<-ball_radius, rl.Fade(rl.RED, 1.0 - ball_alpha))
