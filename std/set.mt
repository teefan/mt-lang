...
pub def release[T](items: ref[HashSet[T]]) -> void:
    map.release[T, bool](addr(value(items).items))
    return

pub def add[T](items: ref[HashSet[T]], value_item: T) -> void:
    map.put[T, bool](addr(value(items).items), value_item, true)
    return

pub def try_add[T](items: ref[HashSet[T]], value_item: T) -> bool:
    return map.try_put[T, bool](addr(value(items).items), value_item, true)
...
pub def remove[T](items: ref[HashSet[T]], value_item: T) -> bool:
    return map.remove[T, bool](addr(value(items).items), value_item)
