...
public function create(seed: ulong) -> Random:
    var generator = Random(state = 0)
    reseed(ref_of(generator), seed)
    return generator


public function reseed(generator: ref[Random], seed: ulong) -> void:
...
public function next_ulong(generator: ref[Random]) -> ulong:
...
public function next_uint(generator: ref[Random]) -> uint:
    return uint<-(next_ulong(generator) >> 32)


public function next_bool(generator: ref[Random]) -> bool:
