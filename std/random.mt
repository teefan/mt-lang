...
pub def reseed(generator: ref[Random], seed: u64) -> void:
    ...
    if value(generator).state == 0:
        value(generator).state = cast[u64](0x9e3779b97f4a7c15)
    return

pub def next_u64(generator: ref[Random]) -> u64:
    var state = value(generator).state
    state = state + cast[u64](0x9e3779b97f4a7c15)
    value(generator).state = state

    var mixed = state
    mixed = (mixed ^ (mixed >> 30)) * cast[u64](0xbf58476d1ce4e5b9)
    mixed = (mixed ^ (mixed >> 27)) * cast[u64](0x94d049bb133111eb)
    return mixed ^ (mixed >> 31)

pub def next_u32(generator: ref[Random]) -> u32:
    return cast[u32](next_u64(generator) >> 32)

pub def next_bool(generator: ref[Random]) -> bool:
    return (next_u64(generator) & cast[u64](1)) == cast[u64](1)

pub def range_usize(generator: ref[Random], upper_bound: usize) -> usize:
    if upper_bound == 0:
        panic(c"random.range_usize upper_bound must be positive")

    return cast[usize](next_u64(generator) % cast[u64](upper_bound))
...
pub def range_i32(generator: ref[Random], min_value: i32, max_value: i32) -> i32:
    ...
    let width = cast[usize](cast[i64](max_value) - cast[i64](min_value) + 1)
    return min_value + cast[i32](range_usize(generator, width))
