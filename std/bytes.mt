...
pub def clear(buffer: ref[Buffer]) -> void:
    vec.clear[u8](addr(value(buffer).items))
    return

pub def release(buffer: ref[Buffer]) -> void:
    vec.release[u8](addr(value(buffer).items))
    return

pub def try_reserve(buffer: ref[Buffer], min_capacity: usize) -> bool:
    return vec.try_reserve[u8](addr(value(buffer).items), min_capacity)

pub def reserve(buffer: ref[Buffer], min_capacity: usize) -> void:
    vec.reserve[u8](addr(value(buffer).items), min_capacity)
    return

pub def try_push(buffer: ref[Buffer], byte: u8) -> bool:
    return vec.try_push[u8](addr(value(buffer).items), byte)

pub def push(buffer: ref[Buffer], byte: u8) -> void:
    vec.push[u8](addr(value(buffer).items), byte)
    return
...
pub def set(buffer: ref[Buffer], index: usize, byte: u8) -> void:
    vec.set[u8](addr(value(buffer).items), index, byte)
    return
...
pub def append(buffer: ref[Buffer], bytes: span[u8]) -> void:
    ...
    while index < bytes.len:
        unsafe:
            push(buffer, deref(bytes.data + index))
        index += 1
    return
