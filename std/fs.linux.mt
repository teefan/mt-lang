...
function release_string_values(values: ref[vec.Vec[string.String]]) -> void:
    ...
    while index < values.len():
        ...
        unsafe:
            var owned = read(ptr[string.String]<-value_ptr)
            owned.release()
...
methods Entries:
    ...
    public function get(index: ptr_uint) -> maybe.Maybe[str]:
        ...
        unsafe:
            return maybe.Maybe[str].some(value= read(ptr[string.String]<-value_ptr).as_str())
    ...
    public function contains(name: str) -> bool:
        ...
        while index < this.values.len():
            ...
            unsafe:
                if read(ptr[string.String]<-value_ptr).as_str().equal(name):
                    return true
