...
async function recv_first_response(
    socket: net.UdpSocket,
    candidates: vec.Vec[Candidate]
    ...
    return Result[PunchResult, Error].failure(
        error = punch_error(err_timeout, "punch recv timed out")
    )
...
public async function punch(
    ...
    let send_result = await send_burst(socket, remote_candidates, probe)
    match send_result:
        Result.failure as sp:
            return Result[PunchResult, Error].failure(error = sp.error)
        Result.success:
            pass

    return await recv_first_response(socket, remote_candidates)
