...
class MilkTeaStdStrCollectionsTest < Minitest::Test
  def test_host_runtime_executes_borrowed_string_map_and_set
    ...
    source = [
      ...
      "    defer str_map.release[i32](ref_of(symbols))",
      "    var seen = str_set.create()",
      "    defer str_set.release(ref_of(seen))",
      "",
      "    str_map.put[i32](ref_of(symbols), \"Token\", 11)",
      "    str_map.put[i32](ref_of(symbols), \"Parser\", 23)",
      "    str_map.put[i32](ref_of(symbols), \"Token\", 13)",
      "    str_set.add(ref_of(seen), \"module\")",
      "    str_set.add(ref_of(seen), \"module\")",
      "    str_set.add(ref_of(seen), \"import\")",
      "",
      "    var token = 0",
      "    if not str_map.get_into[i32](symbols, \"Token\", ref_of(token)):",
      "        return 1",
