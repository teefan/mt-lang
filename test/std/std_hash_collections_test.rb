...
class MilkTeaStdHashCollectionsTest < Minitest::Test
  def test_host_runtime_executes_hash_map_put_get_update_remove
    ...
    source = [
      ...
      "    defer map.release[u64, i32](ref_of(scores))",
      "",
      "    map.put[u64, i32](ref_of(scores), 1, 10)",
      "    map.put[u64, i32](ref_of(scores), 2, 20)",
      "    map.put[u64, i32](ref_of(scores), 1, 15)",
      "",
      "    var first = 0",
      "    var second = 0",
      "    if not map.get_into[u64, i32](scores, 1, ref_of(first)):",
      "        return 1",
      "    if not map.get_into[u64, i32](scores, 2, ref_of(second)):",
      "        return 2",
      "    if map.contains[u64, i32](scores, 3):",
      "        return 3",
      "    if not map.remove[u64, i32](ref_of(scores), 1):",
      "        return 4",
  ...
  def test_host_runtime_executes_hash_map_with_string_keys_and_hash_set
    ...
    source = [
      ...
      "    defer map.release[str, i32](ref_of(names))",
      "    var ids = set.create[i32](hash_i32, equal_i32)",
      "    defer set.release[i32](ref_of(ids))",
      "",
      "    map.put[str, i32](ref_of(names), \"tea\", 7)",
      "    map.put[str, i32](ref_of(names), \"milk\", 5)",
      "    set.add[i32](ref_of(ids), 10)",
      "    set.add[i32](ref_of(ids), 20)",
      "    set.add[i32](ref_of(ids), 10)",
      "",
      "    var tea = 0",
      "    if not map.get_into[str, i32](names, \"tea\", ref_of(tea)):",
      "        return 1",
      "    if not set.contains[i32](ids, 20):",
      "        return 2",
      "    if not set.remove[i32](ref_of(ids), 10):",
      "        return 3",
