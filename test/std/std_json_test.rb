...
class MilkTeaStdJsonTest < Minitest::Test
  def test_host_runtime_executes_json_lexer
    ...
    source = [
      ...
      "    if not item.is_ok:",
      "        return 100 + i32<-item.error",
      "    if item.value.kind != kind:",
      "        return 50 + i32<-item.value.kind",
      "    return i32<-kind",
      "",
      "def main() -> i32:",
  ...
  def test_host_runtime_executes_json_writer
    ...
    source = [
      ...
      "    json.append_i32(ref_of(output), -7)",
      "    let view = output.as_str()",
      "    if view.len != 12:",
      "        return 1",
      "    if text.byte_at(view, 2) != u8<-92:",
      "        return 2",
      "    if text.byte_at(view, 3) != u8<-110:",
      "        return 3",
      "    if text.byte_at(view, 5) != u8<-34:",
      "        return 4",
      "    let total = i32<-view.len + i32<-(text.byte_at(view, 3) - u8<-100) + i32<-(text.byte_at(view, 11) - u8<-50)",
      "    return total",
      "",
    ].join("\n")
