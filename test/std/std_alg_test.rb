...
class MilkTeaStdAlgTest < Minitest::Test
  def test_host_runtime_executes_search_and_predicates
    ...
    source = [
      ...
      "    let items = span[i32](data = ptr_of(ref_of(values[0])), len = 5)",
  ...
  def test_host_runtime_executes_copy_fill_equal_and_sort
    ...
    source = [
      ...
      "    let source = span[i32](data = ptr_of(ref_of(source_values[0])), len = 4)",
      "    let target = span[i32](data = ptr_of(ref_of(target_values[0])), len = 4)",
      "",
