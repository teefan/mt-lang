...
class MilkTeaStdAlgTest < Minitest::Test
  def test_host_runtime_executes_search_and_predicates
    ...
    source = [
      ...
      "    let items = span[i32](data = raw(addr(values[0])), len = 5)",
      "    var found_index: usize = 0",
      "    if not alg.index_of[i32](items, 4, equal_i32, addr(found_index)):",
      "        return 1",
  ...
  def test_host_runtime_executes_copy_fill_equal_and_sort
    ...
    source = [
      ...
      "    let source = span[i32](data = raw(addr(source_values[0])), len = 4)",
      "    let target = span[i32](data = raw(addr(target_values[0])), len = 4)",
      "",
