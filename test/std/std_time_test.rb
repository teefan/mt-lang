...
class MilkTeaStdTimeTest < Minitest::Test
  def test_host_runtime_executes_time_helpers
    ...
    source = [
      ...
      "    if not monotonic.is_ok:",
      "        return 21",
      "    if monotonic.value.tv_nsec < 0 or monotonic.value.tv_nsec >= ptr_int<-1000000000:",
      "        return 22",
      "    if time.timespec_to_nanoseconds(monotonic.value) < 0:",
      "        return 23",
      "    let realtime = time.realtime_time()",
      "    if not realtime.is_ok:",
      "        return 24",
      "    if realtime.value.tv_nsec < 0 or realtime.value.tv_nsec >= ptr_int<-1000000000:",
      "        return 25",
      "    if time.timespec_to_seconds(realtime.value) <= 0.0:",
      "        return 26",
