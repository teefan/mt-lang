...
class MilkTeaStdLibuvRuntimeTest < Minitest::Test
  def test_host_runtime_executes_libuv_timer_helpers
    ...
    source = [
      ...
      "var fired: int = 0",
      "",
      "function on_close(handle: ptr[uv.uv_handle_t]) -> void:",
      "    return",
      "",
      "function on_timer(timer: ptr[uv.uv_timer_t]) -> void:",
      "    fired += 1",
      "    uv.timer_stop(timer)",
      "    unsafe:",
      ...
      "        status.Status.err:",
      "            return 1",
      "        status.Status.ok as loop_payload:",
      "            var loop = loop_payload.value",
      "",
      "            let timer_result = rt.create_timer(loop)",
      "            if status.is_err(timer_result):",
      ...
      "                    rt.loop_release(ref_of(loop))",
      "                    return 2",
      "                status.Status.ok as timer_payload:",
      "                    var timer = timer_payload.value",
      "",
  ...
  def test_host_runtime_executes_libuv_queue_work_helpers
    ...
    source = [
      ...
      "",
      "function on_work(req: ptr[uv.uv_work_t]) -> void:",
      "    work_total += 10",
      "    return",
      "",
      "function after_work(req: ptr[uv.uv_work_t], status: int) -> void:",
      "    if status == 0:",
      "        work_total += 1",
      "    else:",
      "        work_total = -100",
  ...
  def test_host_runtime_executes_libuv_tcp_helpers
    ...
    source = [
      ...
      "    return",
      "",
      "function close_tcp(handle: ptr[uv.uv_tcp_t]?) -> void:",
      "    if handle == null:",
      "        return",
      "    unsafe:",
      ...
      "    return",
      "",
      "function on_connect(req: ptr[uv.uv_connect_t], status: int) -> void:",
      "    if status == 0:",
      "        connected = 1",
      ...
      "    return",
      "",
      "function on_connection(server: ptr[uv.uv_stream_t], status: int) -> void:",
      "    if status != 0:",
      "        accepted = -10",
      "        close_tcp(accepted_handle)",
      "        close_tcp(server_handle)",
      ...
      "            if status.is_err(server_result):",
      "                rt.loop_release(ref_of(loop))",
      "                return 2",
      "            match server_result:",
      "                status.Status.err:",
      ...
      "                                    rt.loop_release(ref_of(loop))",
      "                                    return 4",
      "                                status.Status.ok as accepted_payload:",
      "                                    var accepted_client = accepted_payload.value",
      "",
      "                                    unsafe:",
      ...
      "                                            rt.loop_release(ref_of(loop))",
      "                                            return 6",
      "                                        status.Status.ok as port_payload:",
      ...
      "                                                rt.loop_release(ref_of(loop))",
      "                                                return 7",
      "                                            if rt.tcp_connect_ipv4(connect_request, client, \"127.0.0.1\", port_payload.value, on_connect) != 0:",
      "                                                rt.request_release(ref_of(connect_request))",
      "                                                rt.handle_release(ref_of(accepted_client))",
      "                                                rt.handle_release(ref_of(client))",
      "                                                rt.handle_release(ref_of(server))",
  ...
  def test_host_runtime_executes_libuv_fs_helpers
    ...
    Dir.mktmpdir("milk-tea-std-libuv-fs") do |dir|
      ...
      source = [
        ...
        "",
        "function on_fs(req: ptr[uv.uv_fs_t]) -> void:",
        "    return",
        "",
        "function main() -> int:",
        "    let loop_result = rt.create_loop()",
