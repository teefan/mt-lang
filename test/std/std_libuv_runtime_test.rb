...
class MilkTeaStdLibuvRuntimeTest < Minitest::Test
  def test_host_runtime_executes_libuv_timer_helpers
    ...
    source = [
      ...
      "    uv.timer_stop(timer)",
      "    unsafe:",
      "        uv.close(cast[ptr[uv.uv_handle_t]](timer), on_close)",
      "    return",
      "",
      "def main() -> i32:",
  ...
  def test_host_runtime_executes_libuv_tcp_helpers
    ...
    source = [
      ...
      "        return",
      "    unsafe:",
      "        let raw_handle = cast[ptr[uv.uv_handle_t]](handle)",
      "        if uv.is_closing(cast[const_ptr[uv.uv_handle_t]](raw_handle)) == 0:",
      ...
      "        return",
      "    unsafe:",
      "        let accepted_client = cast[ptr[uv.uv_tcp_t]](maybe_accepted)",
      "        if uv.accept(server, cast[ptr[uv.uv_stream_t]](accepted_client)) != 0:",
      "            accepted = -30",
      "            close_tcp(accepted_handle)",
      "            close_tcp(client_handle)",
      "            close_tcp(server_handle)",
      ...
      "",
      "    unsafe:",
      "        server_handle = cast[ptr[uv.uv_tcp_t]?](rt.handle_ptr(server))",
      "        accepted_handle = cast[ptr[uv.uv_tcp_t]?](rt.handle_ptr(accepted_client))",
      "        client_handle = cast[ptr[uv.uv_tcp_t]?](rt.handle_ptr(client))",
      "",
  ...
  def test_host_runtime_executes_libuv_fs_helpers
    ...
    Dir.mktmpdir("milk-tea-std-libuv-fs") do |dir|
      ...
      source = [
        ...
        "    if rt.loop_run_default(loop) != 0:",
        "        return 3",
        "    let file = cast[i32](rt.fs_result(temp_request))",
        "    if file < 0:",
        "        return 4",
        ...
        "",
        "    unsafe:",
        "        let write_ptr = cast[ptr[u8]](write_bytes)",
        ...
        "    if rt.loop_run_default(loop) != 0:",
        "        return 13",
        "    if rt.fs_result(close_request) != cast[isize](0):",
        "        return 14",
        ...
        "    if rt.loop_run_default(loop) != 0:",
        "        return 16",
        "    if rt.fs_result(unlink_request) != cast[isize](0):",
        "        return 17",
