...
class MilkTeaStdLibuvRuntimeTest < Minitest::Test
  ...
  def test_host_runtime_executes_libuv_queue_work_helpers
    ...
    source = [
      ...
      "    return",
      "",
      "def after_work(req: ptr[uv.uv_work_t], status: int) -> void:",
      "    if status == 0:",
      "        work_total += 1",
      "    else:",
      "        work_total = -100",
      ...
      "        rt.request_release(ref_of(request))",
      "        rt.loop_release(ref_of(loop))",
      "        return 2",
      "    if rt.loop_run_default(loop) != 0:",
      "        rt.request_release(ref_of(request))",
  ...
  def test_host_runtime_executes_libuv_tcp_helpers
    ...
    source = [
      ...
      "    return",
      "",
      "def on_connect(req: ptr[uv.uv_connect_t], status: int) -> void:",
      "    if status == 0:",
      "        connected = 1",
      ...
      "    return",
      "",
      "def on_connection(server: ptr[uv.uv_stream_t], status: int) -> void:",
      "    if status != 0:",
      "        accepted = -10",
      "        close_tcp(accepted_handle)",
      "        close_tcp(server_handle)",
      ...
      "        rt.handle_release(ref_of(server))",
      "        rt.loop_release(ref_of(loop))",
      "        return 3",
      ...
      "        rt.handle_release(ref_of(client))",
      "        rt.handle_release(ref_of(server))",
      "        rt.loop_release(ref_of(loop))",
      "        return 4",
      ...
      "    if rt.tcp_bind_ipv4(server, \"127.0.0.1\", 0, 0, ref_of(scratch)) != 0:",
      "        rt.request_release(ref_of(connect_request))",
      "        rt.handle_release(ref_of(accepted_client))",
      "        rt.handle_release(ref_of(client))",
      "        rt.handle_release(ref_of(server))",
      "        rt.loop_release(ref_of(loop))",
      "        return 5",
      ...
      "        rt.loop_release(ref_of(loop))",
      "        return 7",
      "    if rt.tcp_connect_ipv4(connect_request, client, \"127.0.0.1\", port_result.value, on_connect, ref_of(scratch)) != 0:",
      "        rt.request_release(ref_of(connect_request))",
      "        rt.handle_release(ref_of(accepted_client))",
      "        rt.handle_release(ref_of(client))",
      "        rt.handle_release(ref_of(server))",
  ...
  def test_host_runtime_executes_libuv_fs_helpers
    ...
    Dir.mktmpdir("milk-tea-std-libuv-fs") do |dir|
      ...
      source = [
        ...
        "",
        "    if rt.fs_mkstemp(loop, temp_request, #{template.inspect}, on_fs, ref_of(scratch)) != 0:",
        "        return 2",
        "    if rt.loop_run_default(loop) != 0:",
        "        return 3",
        "    let file = int<-rt.fs_result(temp_request)",
        "    if file < 0:",
        "        return 4",
        ...
        "",
        "    unsafe:",
        "        let write_ptr = ptr[ubyte]<-write_bytes",
        "        read(write_ptr + 0) = 77",
        "        read(write_ptr + 1) = 84",
        "        read(write_ptr + 2) = 45",
        "        read(write_ptr + 3) = 76",
        "",
        "        let write_view = sp.from_ptr[ubyte](write_ptr, 4)",
        "        if rt.fs_write(loop, write_request, file, write_view, 0, on_fs) != 0:",
        "            return 5",
        "        if rt.loop_run_default(loop) != 0:",
        "            return 6",
        "        if rt.fs_result(write_request) != ptr_int<-4:",
        "            return 7",
        "        rt.fs_cleanup(write_request)",
        "",
        "        let read_ptr = ptr[ubyte]<-read_bytes",
        "        let read_view = sp.from_ptr[ubyte](read_ptr, 4)",
        "        if rt.fs_read(loop, read_request, file, read_view, 0, on_fs) != 0:",
        "            return 8",
        "        if rt.loop_run_default(loop) != 0:",
        "            return 9",
        "        if rt.fs_result(read_request) != ptr_int<-4:",
        "            return 10",
        "        rt.fs_cleanup(read_request)",
        "",
        "        if read(read_ptr + 0) != 77 or read(read_ptr + 1) != 84 or read(read_ptr + 2) != 45 or read(read_ptr + 3) != 76:",
        "            return 11",
        "",
        ...
        "    if rt.loop_run_default(loop) != 0:",
        "        return 13",
        "    if rt.fs_result(close_request) != ptr_int<-0:",
        "        return 14",
        "    rt.fs_cleanup(close_request)",
        "",
        "    if rt.fs_unlink(loop, unlink_request, temp_path.as_str(), on_fs, ref_of(scratch)) != 0:",
        "        return 15",
        "    if rt.loop_run_default(loop) != 0:",
        "        return 16",
        "    if rt.fs_result(unlink_request) != ptr_int<-0:",
        "        return 17",
        "    rt.fs_cleanup(unlink_request)",
        "",
        "    rt.request_release(ref_of(unlink_request))",
        "    rt.request_release(ref_of(close_request))",
        "    rt.request_release(ref_of(read_request))",
        "    rt.request_release(ref_of(write_request))",
        "    rt.request_release(ref_of(temp_request))",
        "    if rt.loop_release(ref_of(loop)) != 0:",
        "        return 18",
        "    return 4",
        "",
