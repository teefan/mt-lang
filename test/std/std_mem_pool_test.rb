...
class MilkTeaStdMemPoolTest < Minitest::Test
  ...
  def test_host_runtime_executes_typed_pool_helpers
    ...
    source = [
      ...
      "    defer objects.release()",
      "",
      "    let first = pool.alloc[Pair](ref_of(objects))",
      "    let second = pool.alloc[Pair](ref_of(objects))",
      "    let third = pool.alloc[Pair](ref_of(objects))",
      "    if first == null or second == null:",
      "        return 1",
      "    if third != null:",
      "        return 2",
      ...
      "            return 3",
      "",
      "    if not pool.release[Pair](ref_of(objects), first):",
      "        return 4",
      "    if pool.release[Pair](ref_of(objects), first):",
      "        return 5",
      "    if not pool.release[Pair](ref_of(objects), second):",
      "        return 6",
      "",
      "    return 0",
      "",
