...
class MilkTeaStdJsoncTest < Minitest::Test
  ...
  def test_host_runtime_normalizes_jsonc_for_json_lexer
    ...
    source = [
      ...
      "",
      "function take_kind(lexer: ref[json.Lexer], kind: json.TokenKind, code: int) -> int:",
      ...
      "    return code",
      "",
      "function take_text(lexer: ref[json.Lexer], kind: json.TokenKind, expected: str, code: int) -> int:",
      "    let item = json.next(lexer)",
      "    if status.is_err(item):",
      "        return code",
      "    match item:",
