...
class MilkTeaGlfwIdiomaticTestPortsTest < Minitest::Test
  def test_idiomatic_test_ports_use_wrapper_surface_for_gl_and_glfw
    idiomatic_test_port_paths.each do |path|
      source = File.read(path)

      assert_match(/^module examples\.idiomatic\.glfw\.tests\./, source)
  ...
  def test_idiomatic_test_ports_build_with_host_compiler
    compiler = ENV.fetch("CC", "cc")
    skip "C compiler not available: #{compiler}" unless compiler_available?(compiler)

    manifest = idiomatic_test_port_paths
    refute_empty manifest

    Dir.mktmpdir("milk-tea-glfw-idiomatic-test-ports-build") do |dir|
  ...
  def idiomatic_test_port_paths
    Dir[File.join(examples_root, "idiomatic", "glfw", "tests", "**", "*.mt")].sort
  end

  def announce_build_progress(current, total, relative_path)
    $stdout.puts("[glfw_idiomatic_test_ports_test #{current}/#{total}] #{relative_path}")
    $stdout.flush
  end
