...
class MilkTeaImportedBindingsTest < Minitest::Test
  ...
  def test_checked_in_raylib_binding_matches_policy_and_loads
    ...
    assert_match(/^pub def draw_spline_linear\(points: span\[Vector2\], thick: f32, color: Color\) -> void:$/, source)
    assert_match(/^pub def draw_triangle_strip\(points: span\[Vector2\], color: Color\) -> void:$/, source)
    assert_match(/^pub def update_texture_from_image\(texture: Texture, image: Image\) -> void:$/, source)
    assert_match(/^pub def update_texture_from_image_frame\(texture: Texture, image: Image, frame_index: i32\) -> void:$/, source)
    assert_match(/^pub def check_collision_point_poly\(point: Vector2, points: span\[Vector2\]\) -> bool:$/, source)
    ...
    assert_match(/^pub def load_codepoints\(text: str\) -> CodepointList:$/, source)
    assert_match(/^pub def text_split\(text: str, delimiter: char\) -> TextSplitResult:$/, source)
    assert_match(/^pub def file_path_at\(files: FilePathList, index: i32\) -> cstr:$/, source)
    assert_match(/^pub def codepoint_in_text_at\(text: str, byte_index: i32\) -> TextCodepoint:$/, source)
