...
class MilkTeaImportedBindingsTest < Minitest::Test
  ...
  def test_checked_in_libuv_binding_matches_policy_and_loads
    ...
    assert_match(/^pub foreign def run\(arg_0: ptr\[uv_loop_t\], mode: uv_run_mode\) -> i32 = c\.uv_run$/, source)
  ...
  def test_checked_in_sdl3_binding_matches_policy_and_loads
    ...
    assert_match(/^pub foreign def create_window\(title: str as cstr, w: i32, h: i32, flag_bits: usize\) -> ptr\[Window\] = c\.SDL_CreateWindow$/, source)
  ...
  def test_checked_in_raylib_binding_matches_policy_and_loads
    ...
    assert_match(/^pub foreign def load_font_data\(file_data: const_ptr\[u8\], data_size: i32, font_size: i32, codepoints: ptr\[i32\]\?, codepoint_count: i32, kind: FontType, out glyph_count: i32\) -> ptr\[GlyphInfo\] = c\.LoadFontData$/, source)
