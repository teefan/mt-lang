...
class MilkTeaImportedBindingsTest < Minitest::Test
  ...
  def test_checked_in_raylib_binding_matches_policy_and_loads
    ...
    assert_match(/^public foreign function get_world_to_screen_2d\(position: vec2, camera: Camera2D\) -> vec2 = c\.GetWorldToScreen2D$/, source)
    assert_match(/^public foreign function get_screen_to_world_2d\(position: vec2, camera: Camera2D\) -> vec2 = c\.GetScreenToWorld2D$/, source)
    assert_match(/^public foreign function get_camera_matrix_2d\(camera: Camera2D\) -> mat4 = c\.GetCameraMatrix2D$/, source)
    ...
    assert_match(/^public foreign function draw_spline_linear_ptr\(points: const_ptr\[vec2\], point_count: int, thick: float, color: Color\) -> void = c\.DrawSplineLinear$/, source)
    ...
    assert_match(/^public foreign function image_draw_text_ex\(inout dst: Image, font: Font, text: str as cstr, position: vec2, font_size: float, spacing: float, tint: Color\) -> void = c\.ImageDrawTextEx$/, source)
    assert_match(/^public foreign function draw_text_ex\(font: Font, text: str as cstr, position: vec2, font_size: float, spacing: float, tint: Color\) -> void = c\.DrawTextEx$/, source)
    ...
    assert_match(/^public foreign function draw_line_3d\(start_pos: vec3, end_pos: vec3, color: Color\) -> void = c\.DrawLine3D$/, source)
    assert_match(/^public foreign function draw_point_3d\(position: vec3, color: Color\) -> void = c\.DrawPoint3D$/, source)
    assert_match(/^public foreign function draw_circle_3d\(center: vec3, radius: float, rotation_axis: vec3, rotation_angle: float, color: Color\) -> void = c\.DrawCircle3D$/, source)
    assert_match(/^public foreign function draw_triangle_3d\(v1: vec3, v2: vec3, v3: vec3, color: Color\) -> void = c\.DrawTriangle3D$/, source)
  ...
  def test_checked_in_raymath_binding_matches_policy_and_loads
    ...
    assert_match(/^public foreign function vector3_to_float_v\(v: vec3\) -> array\[float, 3\] = c\.Vector3ToFloatV\(v\)\.v$/, source)
    assert_match(/^public foreign function matrix_to_float_v\(mat: mat4\) -> array\[float, 16\] = c\.MatrixToFloatV\(mat\)\.v$/, source)
    assert_match(/^public foreign function vector2_zero\(\) -> vec2 = c\.Vector2Zero$/, source)
    refute_match(/^public foreign function vector_2zero\(\) -> vec2 = c\.Vector2Zero$/, source)
    refute_match(/^public foreign function vector_2_zero\(\) -> vec2 = c\.Vector2Zero$/, source)
    assert_match(/^public foreign function vector3_ortho_normalize\(inout v1: vec3, inout v2: vec3\) -> void = c\.Vector3OrthoNormalize$/, source)
    assert_match(/^public foreign function quaternion_to_axis_angle\(q: quat, out axis: vec3, out angle: float\) -> void = c\.QuaternionToAxisAngle$/, source)
    assert_match(/^public foreign function matrix_decompose\(mat: mat4, out translation: vec3, out rotation: quat, out scale: vec3\) -> void = c\.MatrixDecompose$/, source)
    refute_match(/^extending vec2:$/, source)
    refute_match(/^extending mat4:$/, source)
    refute_match(/^extending quat:$/, source)
  end
