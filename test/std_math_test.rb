...
class MilkTeaStdMathTest < Minitest::Test
  ...
  def test_host_runtime_executes_float_std_math_helpers
    ...
    source = [
      ...
      "",
      "def main() -> i32:",
      "    let low = math.min(math.pi_f32, math.tau_f32)",
      "    let high = math.max(math.pi_f32, math.tau_f32)",
      "    let halfway = math.lerp_f32(math.pi_f32, math.tau_f32, 0.5)",
      "    let clamped = math.clamp(halfway, math.pi_f32, math.tau_f32)",
      "    let delta = math.abs(clamped - 4.712389)",
      "    if low == math.pi_f32 and high == math.tau_f32 and delta < 0.01:",
      "        return 0",
      "    return 1",
      "",
    ].join("\n")
