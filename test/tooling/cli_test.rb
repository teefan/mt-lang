...
class MilkTeaCliTest < Minitest::Test
  ...
  def test_format_command_ignore_generated_skips_generated_files
    Dir.mktmpdir("milk-tea-cli-fmt-ignore-generated") do |dir|
      generated_path = File.join(dir, "generated.mt")
      regular_path = File.join(dir, "regular.mt")
      File.write(generated_path, "# generated by mtc bindgen from /tmp/header.h\nfunction  generated()->int:\n    return 0\n")
      File.write(regular_path, "function  regular()->int:\n    return 0\n")

      out = StringIO.new
      err = StringIO.new

      status = MilkTea::CLI.start(["format", generated_path, regular_path, "--check", "--ignore-generated"], out:, err:)

      assert_equal 1, status
      assert_equal "", err.string
      refute_match(/needs formatting .*generated\.mt/, out.string)
      assert_match(/needs formatting .*regular\.mt/, out.string)
      assert_match(/skipped 1 generated file\(s\)/, out.string)
    end
  end
  ...
  def test_lint_command_ignore_generated_skips_generated_file
    Dir.mktmpdir("milk-tea-cli-lint-ignore-generated") do |dir|
      generated_path = File.join(dir, "generated.mt")
      File.write(generated_path, <<~MT)
      ...
      status = MilkTea::CLI.start(["lint", generated_path, "--ignore-generated"], out:, err:)

      assert_equal 0, status
      assert_equal "", err.string
      assert_match(/clean .*generated\.mt/, out.string)
      refute_match(/unused-local/, out.string)
    end
  end
