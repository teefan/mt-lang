  ...
  def test_run_with_host_compiler_executes_program_using_span_str_foreign_list_marshalling
    ...
    begin
      ...
      File.write(sample_path, [
        ...
        "import std.c.libc as libc",
        "",
        "pub foreign def first(labels: span[str] as span[ptr[char]]) -> int = libc.atoi(labels[0])",
        "pub foreign def second(labels: span[str] as span[ptr[char]]) -> int = libc.atoi(labels[1])",
        "",
      ].join("\n"))
  ...
  def test_run_with_host_compiler_executes_program_using_nested_imported_foreign_calls
    ...
    begin
      ...
      File.write(sample_path, [
        ...
        "pub foreign def first(labels: span[str] as span[ptr[char]]) -> int = libc.atoi(labels[0])",
        "pub foreign def second(labels: span[str] as span[ptr[char]]) -> int = libc.atoi(labels[1])",
        "pub foreign def same_div(value: int) -> libc.div_t = libc.div(value, value)",
        "",
      ].join("\n"))
