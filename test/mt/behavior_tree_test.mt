...
@[test]
function test_behavior_tree_nodes() -> void:
    ...
    expect(tree.node_count() == 7, "7 nodes")
    expect(tree.set_root(root))
    expect(tree.add_child(root, chase))
    expect(tree.add_child(root, patrol_repeat))
    expect(tree.add_child(chase, can_see))
    expect(tree.add_child(chase, approach))
    expect(tree.add_child(chase, attack))
    expect(tree.add_child(patrol_repeat, patrol_leaf))
    ...
    expect(tree.tick(context) == bt.Status.running, "first tick running")
    expect_eq(context.patrol_count, 1)

    expect(tree.tick(context) == bt.Status.success, "second tick success")
    expect_eq(context.patrol_count, 2)

    tree.reset()
    context.target_visible = true
    expect(tree.tick(context) == bt.Status.running, "chase tick running")
    expect_eq(context.attack_count, 0)

    expect(tree.tick(context) == bt.Status.success, "chase tick success")
    expect_eq(context.attack_count, 1)
    ...
    expect(inverter_tree.set_root(inverter_root))
    expect(inverter_tree.add_child(inverter_root, false_check))
    expect(inverter_tree.tick(context) == bt.Status.success, "inverter success")
    expect_eq(context.failed_checks, 1)
    ...
    expect(retry_tree.set_root(until_root))
    expect(retry_tree.add_child(until_root, retry_leaf))
    expect(retry_tree.tick(context) == bt.Status.running, "retry tick 1 running")
    expect(retry_tree.tick(context) == bt.Status.running, "retry tick 2 running")
    expect(retry_tree.tick(context) == bt.Status.success, "retry tick 3 success")
    expect_eq(context.until_success_attempts, 3)
