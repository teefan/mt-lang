...
@[test]
function test_fmt_basic_formatting() -> void:
    ...
    expect(view.len == 35z, "formatted length == 35")
    let total = int<-view.len + byte_at(view, 34)
    expect_eq(total, 154)


@[test]
function test_fmt_format_literals() -> void:
    var scratch = arena.create(64)
    defer: scratch.release()
    let delta: short = -42
    let small: ubyte = 7
    ...
    defer: output.release()
    expect_eq(int<-output.len(), 37)


@[test]
function test_fmt_direct_string_sink_format_literals() -> void:
    let value = 7
    var output = string.String.create()
    defer: output.release()
    output.assign(f"value=#{value}")
    output.append(f" ok=#{true}")
    expect_eq(int<-output.len(), 15)


@[test]
function test_fmt_explicit_builder_format_sinks() -> void:
    var scratch = arena.create(64)
    defer: scratch.release()
    let value: uint = 26
    ...
    expect(output.as_str() == "seed hex=1a oct=32", "hex/oct append")

    fmt.assign_format(ref_of(output), f"ratio=#{ratio:.2} ok=#{true}")
    expect(output.as_str() == "ratio=3.50 ok=true", "ratio assign")

    output.append_format(f" raw=#{scratch.to_cstr("wow")} bin=#{value:b}")
    expect(output.as_str() == "ratio=3.50 ok=true raw=wow bin=11010", "raw/bin append")

    output.assign_format(f"HEX=#{value:X}")
    expect(output.as_str() == "HEX=1A", "HEX assign")


@[test]
function test_fmt_preserves_aliasing() -> void:
    var output = string.String.from_str("abc")
    defer: output.release()

    output.assign_format(f"#{output.as_str()}x")
    expect(output.as_str() == "abcx", "aliased assign")

    output.append_format(f"|#{output.as_str()}")
    expect(output.as_str() == "abcx|abcx", "aliased append")


@[test]
function test_fmt_str_buffer_format_sinks() -> void:
    let value: uint = 26
    let ratio: double = 3.5
    var buffer: str_buffer[64]

    buffer.assign_format(f"#{value:x}")
    expect(buffer.as_str() == "1a", "buffer hex assign")

    buffer.append_format(f"|#{ratio:.2}")
    expect(buffer.as_str() == "1a|3.50", "buffer ratio append")

    buffer.assign("abc")
    buffer.assign_format(f"#{buffer.as_str()}x")
    expect(buffer.as_str() == "abcx", "buffer aliased assign")

    buffer.append_format(f"|#{buffer.as_cstr()}")
    expect(buffer.as_str() == "abcx|abcx", "buffer aliased append")


@[test]
function test_fmt_custom_format_hooks() -> void:
    let point = Point(x = 2, y = 3)
    let text = f"point=#{point}"
    expect(text == "point=(2, 3)", "custom hook in f-string")

    var output = string.String.create()
    defer: output.release()
    output.append_format(f"[#{point}]")
    expect(output.as_str() == "[(2, 3)]", "custom hook append")

    fmt.assign_format(ref_of(output), f"#{point}!")
    expect(output.as_str() == "(2, 3)!", "custom hook assign")

    var buffer: str_buffer[64]
    buffer.assign_format(f"<#{point}>")
    expect(buffer.as_str() == "<(2, 3)>", "custom hook buffer")


@[test]
function test_fmt_float_format_literals() -> void:
    let ratio: float = 2.5
    let scale: double = 0.125
    var output = fmt.format(f"ratio=#{ratio} scale=#{scale}")
    defer: output.release()
    expect_eq(int<-output.len(), 21)


@[test]
function test_fmt_general_format_string_expressions() -> void:
    let count = 7
    let text = f"count=#{count}"
    expect(size(f"ok=#{true}") != 0z, "format expr non-empty")
    expect_eq(int<-text.len, 7)
