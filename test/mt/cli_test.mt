...
function expect_value(match_result: cli.Match, name: str, expected: str) -> void:
    match match_result.value(name):
        Option.some as payload:
            expect(payload.value.equal(expected))
        Option.none:
            expect(false, "missing cli value")


@[test]
function test_cli_parsing_and_help() -> void:
    ...
    match cli.parse(app, args):
        Result.failure as payload:
            ...
            error.release()
            expect(false, "parse with args failed")
        Result.success as payload:
            var parsed = payload.value
            defer: parsed.release()
            expect(not parsed.help_requested())
            match parsed.command():
                Option.some as command_payload:
                    expect(command_payload.value.equal("inspect"))
                Option.none:
                    expect(false, "command missing")
            expect(parsed.flag("verbose"))
            expect_value(parsed, "config", "custom.toml")
            expect_value(parsed, "interval", "250")
            expect(parsed.positionals_len() == 1z, "one positional")
            match parsed.positional(0):
                Option.some as positional_payload:
                    expect(positional_payload.value.equal("cpu"))
                Option.none:
                    expect(false, "positional missing")
    ...
    match cli.parse(app, default_args):
        Result.failure as payload:
            var error = payload.error
            error.release()
            expect(false, "parse defaults failed")
        Result.success as payload:
            var parsed = payload.value
            defer: parsed.release()
            expect(not parsed.flag("verbose"))
    ...
    match cli.parse(app, bad_args):
        Result.failure as payload:
            var error = payload.error
            defer: error.release()
            expect(error.message.as_str().equal("unknown option --wat"))
        Result.success as payload:
            var parsed = payload.value
            parsed.release()
            expect(false, "bad args should fail to parse")
