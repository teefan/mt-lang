...
@[test]
function test_steering_seek() -> void:
    ...
    expect(vec_approx(f, vec2(x = 5.0, y = 0.0), 0.01), "seek toward target")



@[test]
function test_steering_flee() -> void:
    ...
    expect(f.x > 0.0, "flee moves away from threat")



@[test]
function test_steering_arrive() -> void:
    let pos = vec2(x = 0.0, y = 0.0)
    ...
    expect(vec_approx(f, vec2(x = 0.0, y = 0.0), 0.01), "arrive matches velocity at distance")



@[test]
function test_steering_separation() -> void:
    let pos = vec2(x = 0.0, y = 0.0)
    ...
    expect(f.x < 0.0, "separation pushes away from neighbors")



@[test]
function test_steering_alignment() -> void:
    let vel = vec2(x = 0.0, y = 0.0)
    ...
    expect(vec_approx(f, vec2(x = 1.0, y = 0.0), 0.01), "align to neighbors")



@[test]
function test_steering_cohesion() -> void:
    let pos = vec2(x = 0.0, y = 0.0)
    ...
    expect(f.x > 0.0, "cohesion toward center")



@[test]
function test_steering_limit() -> void:
    let f = vec2(x = 10.0, y = 0.0)
    let capped = vec2.limit(f, 5.0)
    expect(vec_approx(capped, vec2(x = 5.0, y = 0.0), 0.01), "force capped at max")



@[test]
function test_steering_limit_under() -> void:
    let f = vec2(x = 3.0, y = 0.0)
    let capped = vec2.limit(f, 5.0)
    expect(vec_approx(capped, f, 0.01), "force unchanged when under limit")
