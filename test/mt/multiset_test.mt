...
@[test]
function test_multiset_operations() -> void:
    ...
    expect(values.distinct_len() == 0, "distinct_len == 0")

    expect(values.insert(Key(value = 3)) == 1, "insert(3) == 1")
    expect(values.add(Key(value = 1), 2) == 2, "add(1, 2) == 2")
    expect(values.add(Key(value = 4), 3) == 3, "add(4, 3) == 3")
    expect(values.insert(Key(value = 1)) == 3, "insert(1) == 3")
    expect(values.insert(Key(value = 2)) == 1, "insert(2) == 1")

    expect(values.len() == 8, "len == 8")
    expect(values.total_count() == 8, "total_count == 8")
    expect(values.distinct_len() == 4, "distinct_len == 4")
    expect(values.count(Key(value = 1)) == 3, "count(1) == 3")
    ...
    for value in values.values():
        unsafe:
            ...
            if value_step == 3 and current != 2:
                value_order_ok = false
        value_step += 1
    expect(value_order_ok)
    expect_eq(value_step, 4)
    ...
    expect_eq(entry_step, 4)
    expect(count_total == 8, "entry counts sum == 8")

    expect(values.remove_one(Key(value = 1)))
    expect(values.count(Key(value = 1)) == 2, "count(1) == 2")
    expect(values.total_count() == 7, "total_count == 7")

    var removed_ok = false
    match values.remove_all(Key(value = 4)):
        Option.none:
            expect(false, "remove_all(4) none")
        Option.some as payload:
            removed_ok = payload.value == 3
    expect(removed_ok)

    expect(values.total_count() == 4, "total_count == 4")
    expect(values.distinct_len() == 3, "distinct_len == 3")
    ...
    expect_eq(difference_step, 1)

    var combined = other.union_with(values)
    defer: combined.release()
    expect(values.is_subset(combined))
    ...
    for entry in symmetric_values:
        unsafe:
            ...
            if symmetric_step == 2 and (current != 5 or entry.count != 1):
                symmetric_order_ok = false
        symmetric_step += 1
    expect(symmetric_order_ok)
    expect_eq(symmetric_step, 3)

    values.clear()
    expect(values.is_empty())
    expect(values.len() == 0, "len == 0 after clear")
    expect(values.distinct_len() == 0, "distinct_len == 0 after clear")
